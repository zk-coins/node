...
| **P2 — zkCoins shared-account primitive** | Implement 2-of-2 MuSig2 shared accounts in `zk-coins/server` (a prerequisite that does not exist today; [`SPEC.md`](./SPEC.md) §3 single-account-per-pubkey model needs extension). Shielded CSV §5.1 has the protocol-level construction. Persistence, recovery-tx pre-signing, capabilities-flag gating. | **L** | Medium — touches account-state model |
