...
| **N=4, q=100** | 1.95 s | 2.26 s | **1.9× faster** | 1.27× | **protocol: MAX_IN_COINS 8→4** |
| N=8, q=48 (64-bit inner) | 1.62 s | 1.93 s | 2.25× | 1.32× | auditor: 64-bit inner FRI |
| **N=4, q=48 (RECOMMENDED, Probe AE composed)** | 1.00 s | **1.31 s** | **3.32× faster** | **1.45× (6.91 s)** | both above |
...
## Revised migration verdict (supersedes the "wash" headline of the full audit)

- **No changes:** `/api/send` wash; mint ~2×, cold-start 38.7×, RSS ~2× remain the wins.
- **With MAX_IN_COINS=4 (operator/protocol decision only):** send-prove **1.9× faster**,
  e2e 1.27× — a clean win with NO open soundness question.
- **With MAX_IN_COINS=4 + 64-bit inner FRI (auditor sign-off):** send-prove **3.3× faster**,
  e2e **1.45×** (6.91 s) — the recommended target config.
- **Field decision:** **BabyBear** (KoalaBear ruled out by AD; Goldilocks forfeits the
  field-driven win and only avoids the SDK bump, Doc 2).
