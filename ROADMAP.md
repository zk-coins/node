...
| 7 | Server: **replace** SP1 path with Plonky2 (no feature flag, no dual backend) | 🟡 in progress — workspace toolchain unification + server-side import migration + CI workflow updated to nightly **DONE** (`00adbb4` + `ee0ef4b` + `19dcecf`, 32 server tests pass, clippy `-D warnings` green for MVP + all-features, coverage gate green on scoped surface). Prover-API integration in `account_server::send_coins` DEFERRED to post-merge of Stage 5d-next-5 aggregator (issue #19). | ~0.5 d after merge | low |
