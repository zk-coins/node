...
| 7 | Server: **replace** SP1 path with Plonky2 (no feature flag, no dual backend) | ✅ done (`c71c9fc`) — `send_coins` wired to the Plonky2 Prover. Off-circuit source-side validation per server-heavy MVP architecture (Stage 5d-next-5 Phase 2 deferred post-MVP per [#19](https://github.com/zk-coins/server/issues/19)). 32 server tests pass; `account_server_tests` + `server_tests` modules remain disabled at the include-point (SP1-specific test fixtures need porting — separate follow-up task, ~3–6 h estimated). | — | — |
