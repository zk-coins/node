...
| 7 | Server: **replace** SP1 path with Plonky2 (no feature flag, no dual backend) | 🟡 in progress — workspace toolchain unification + server-side import migration **DONE** (`00adbb4`, builds clean, 31 server tests pass). Prover-API integration in `account_server::send_coins` DEFERRED to post-merge of Stage 5d-next-5 aggregator (issue #19). | ~0.5 d after merge | low |
