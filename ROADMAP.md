...
| 7 | Server: **replace** SP1 path with Plonky2 (no feature flag, no dual backend) | 🟡 in progress — 🛠 persistence helpers + serde derives done (`b76bd39`); remaining work re-scoped to ~2 d after a 2026-05-17 cutover attempt surfaced semantic mismatches beyond "mechanical renames" (HashDigest type shift, proof.public_values→public_inputs, ProgramInputsBuilder absence, hex::encode-on-HashOut). See [`program-plonky2/STEP7_PREP.md`](program-plonky2/STEP7_PREP.md) "Semantic mismatches that the original inventory missed". | ~2 d | low |
