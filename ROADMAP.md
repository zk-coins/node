...
| 7 | Server: **replace** SP1 path with Plonky2 (no feature flag, no dual backend) | ✅ done (`c71c9fc` + `dac0179` + `d6a3cb9`) — `send_coins` wired to the Plonky2 Prover (off-circuit source-side validation per server-heavy MVP, Stage 5d-next-5 Phase 2 deferred post-MVP per [#19](https://github.com/zk-coins/server/issues/19)). Dockerfile re-introduced (`dac0179`). 42 server tests pass (32 baseline + 10 inline error-path); smoke-test verified end-to-end (`cargo run` + `/health` + `/api/info`, block scanner connects). `account_server_tests` + `server_tests` modules remain disabled at the include-point (SP1-specific test fixtures need porting — separate follow-up task, ~3–6 h estimated). | — | — |
