...
| 7 | Server: **replace** SP1 path with Plonky2 (no feature flag, no dual backend) | ✅ done — `send_coins` wired to the Plonky2 Prover. Initial server cut (`c71c9fc`) used off-circuit source-side validation; this branch refactors it to **in-circuit** via `prove_*_and_sources` now that Stage 5d-next-5 Phase 2b has landed (PR [#23](https://github.com/zk-coins/server/pull/23)). Dockerfile re-introduced (`dac0179`). 106 server tests pass on the MVP build, 119 with `--all-features` (32 baseline + 10 inline error-path in `d6a3cb9` + 64 ported SP1-era fixtures re-enabled in `account_server_tests.rs` / `server_tests.rs` + 13 feature-gated). Smoke-test verified end-to-end (`cargo run` + `/health` + `/api/info`, block scanner connects). | — | — |
